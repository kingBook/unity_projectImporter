...
namespace UnityTools {
    ...
    public class ProjectImporterUI : EditorWindow {
        ...
        private static FileLoader s_fileLoader;
        private static bool s_isLoadXmlComplete;
        ...
        public static async void SaveXml() {
            if (s_xmlDoc == null) return;
            await Task.Run(() => {
                s_xmlDoc.Save(XmlPath);
            });
        }
        ...
        public static void LoadXml() {
            if (s_fileLoader == null) {
                s_fileLoader = new FileLoader();
            }
            s_fileLoader.LoadAsync(XmlPath);
            s_fileLoader.onCompleteAll += OnLoadXmlComplete;
        }
        ...
        private static void OnLoadXmlComplete(byte[][] bytesList) {
            ...
            if (bytes != null) {
                ...
                s_xmlDoc = XmlUtil.CreateXmlDocument(xmlString, false);
            }
            s_isLoadXmlComplete = true;

        }

        public static XmlDocument xmlDocument { get => s_xmlDoc; }
        #endregion
        ...
        private void OnGUI() {
            ...
            EditorGUILayout.BeginVertical();
            {
                m_scrollPosition = EditorGUILayout.BeginScrollView(m_scrollPosition);
                {
                    ...
                    //
                    if (s_xmlDoc != null) {
                        var items = s_xmlDoc.FirstChild.ChildNodes;
        ...
        private void OnDeleteProject(XmlNode item, string projectName) {
            s_xmlDoc.FirstChild.RemoveChild(item);
            SaveXml();
            //删除项目
            ProjectImporterEditor.DeleteProject(projectName);
        }
        ...
        private void AddItemToXml(string projectFolderPath, string projectName, string editorVersion) {
            if (s_xmlDoc == null) {
                s_xmlDoc = XmlUtil.CreateXmlDocument(false);
                s_xmlDoc.AppendChild(s_xmlDoc.CreateElement("Root"));
            }
            var itemElement = s_xmlDoc.CreateElement("Item");
            ...
            s_xmlDoc.FirstChild.AppendChild(itemElement);
            SaveXml();
        }
        ...
        private bool IsAlreadyExists(string projectFolderPath) {
            if (s_xmlDoc != null) {
                XmlNodeList items = s_xmlDoc.FirstChild.ChildNodes;
                int len = items.Count;
                for (int i = 0; i < len; i++) {
